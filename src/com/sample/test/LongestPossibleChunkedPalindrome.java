...
public class LongestPossibleChunkedPalindrome {
	...
	public static void main(String[] args) {
		...
		System.out.println(obj.longestPossibleChunkedPalindrome(str, 0, str));
	}
	
	public int longestPossibleChunkedPalindrome(String str, int palindromeCount, String curr_string) {
		if(curr_string == null || curr_string.isEmpty()) {
			return 0;
		}
		
		if(curr_string.length() == 1) {
		...
		for(int i = 0; i<stringLength/2; i++) {
			if(curr_string.substring(0, i+1).equals(curr_string.substring(stringLength-1-i,stringLength))) {
				return longestPossibleChunkedPalindrome(str, palindromeCount+2, curr_string.substring(i+1,stringLength-1-i));
			}
		}
